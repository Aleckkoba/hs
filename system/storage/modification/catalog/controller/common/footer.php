...
class ControllerCommonFooter extends Controller {
	public function index() {
        ...
        if ($this->config->get('jivosite_status')) { 
          $data['jivosite_widget_id'] = $this->config->get('jivosite_widget_id'); 
          $data['jivosite_status'] = true;
          $data['jlogged'] = $this->customer->isLogged();
          if ($this->customer->isLogged()) {
              $data['jname'] = $this->customer->getFirstName() . ' ' . $this->customer->getLastName();
              $data['jemail'] = $this->customer->getEmail();
              $data['jtel'] = $this->customer->getTelephone();
              $this->load->model('account/address');
              $jaddr = $this->model_account_address->getAddress($this->customer->getAddressId());
              $data['jdesc'] = $jaddr['postcode'] . ' ' . $jaddr['city'] . ' ' . $jaddr['address_1'];
          }
        }
...
$data['ishome']=1;
        ...
        if($social_icons_vt != false){
            $this->document->addStyle('catalog/view/theme/default/stylesheet/social_icons_vt.css');
            $html = '<div class="social_icons_vt">';
                foreach ($social_icons_vt as $key => $icon_vt){
					if($icon_vt){
						$html .= '<a class="icon_vt ' . $key . '" href="'. $icon_vt . '" target = "_blanck"></a>';
					}
				}
            $html .= '</div>';
            $data['social_icons_vt'] = $html;
        }else{
            $data['social_icons_vt'] = false;
        }
        //end social vt
