...
<div class="container-fluid">
    <div class="pull-right">
        ...
        <button onclick="saveSettings();$('#import_form').submit();return false;" data-toggle="tooltip" title="<?php echo $button_next; ?>" class="btn btn-primary" intercom-tracked="run-import"><i class="fa fa-check"></i></button>
        ...
        <button onclick="saveAndExportSettings();return false;" data-toggle="tooltip" title="Export your settings" class="btn btn-info" intercom-tracked="saved-profile"><i class="fa fa-cloud-download"></i></button>
        <a href="<?php echo $back_url;?>" data-toggle="tooltip" title="<?php echo $button_back; ?>" class="btn btn-warning"><i class="fa fa-reply"></i></a>
        <a href="<?php echo $cancel; ?>" data-toggle="tooltip" title="<?php echo $button_cancel; ?>" class="btn btn-danger" intercom-tracked="step-canceled"><i class="fa fa-home"></i></a>
    </div>
    ...
    <ul id="tabs" class="nav nav-tabs"><li class="active"><a data-toggle="tab" href="#tab_import"><?php echo $tab_import; ?></a></li></ul>
        <div id="tab_import">
           <div class="tab-pane active form-horizontal" id="tab_import">
                    <div class="form-group" id="progress-items" style="display:none;">
                        ...
                        <div class="col-md-10">
                            ...
                            <div id="progress-text">
                                ...
                                <a id="log-btn" href="index.php?route=extension/module/total_import/log&user_token=<?php echo $token; ?>">View Log</a>
                            </div>
                        </div>
                    </div>
                                        ...
                                        <div class="well well-sm">
                                            <div class="checkbox">
                                                <label for="reset_products">
                                                    <input type="checkbox" name="reset_products" id="reset_products">
                                                    <?php echo $table_products; ?>
                                                </label>
                                            </div>
                                            <div class="checkbox">
                                                <label for="reset_categories">
                                                    <input type="checkbox" name="reset_categories" id="reset_categories">
                                                    <?php echo $table_categories; ?>
                                                </label>
                                            </div>
                                            <div class="checkbox">
                                                <label for="reset_products">
                                                    <input type="checkbox" name="reset_manufacturers" id="reset_manufacturers">
                                                    <?php echo $table_manufacturers; ?>
                                                </label>
                                            </div>
                                            <div class="checkbox">
                                                <label for="reset_attributes">
                                                    <input type="checkbox" name="reset_attributes" id="reset_attributes">
                                                    <?php echo $table_attributes; ?>
                                                </label>
                                            </div>
                                            <div class="checkbox">
                                                <label for="reset_options">
                                                    <input type="checkbox" name="reset_options" id="reset_options">
                                                    <?php echo $table_options; ?>
                                                </label>
                                            </div>
                                            <div class="checkbox">
                                                <label for="reset_downloads">
                                                    <input type="checkbox" name="reset_downloads" id="reset_downloads">
                                                    <?php echo $table_downloads; ?>
                                                </label>
                                            </div>
                                            <div class="checkbox">
                                                <label for="reset_filters">
                                                    <input type="checkbox" name="reset_filters" id="reset_filters">
                                                    <?php echo $table_filters; ?>
                                                </label>
                                            </div>
                                    </div>
                                    <a onclick="$(this).parent().find(':checkbox').prop('checked', true);" intercom-tracked="reset-store">Select All</a> / <a onclick="$(this).parent().find(':checkbox').prop('checked', false);">Unselect All</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- Size -->
                    <div class="form-group">
                        <label class="col-md-2 control-label"><?php echo $entry_limit_on_import; ?></label>
                        <div class="col-md-10">
                            <input type="text" name="limit_on_import" value="<?php
                            if (isset($limit_on_import)) { echo $limit_on_import;}  else {echo '100';} ?>" class="form-control">
                            <span class="help-block">
                                <?php echo $help_products_per_segment;?>
                                <a href="https://hostjars.zendesk.com/hc/en-us/articles/203573259-Fatal-error-Maximum-execution-time-of-30-seconds-exceeded"><i class="fa fa-question-circle"></i></a>
                            </span>
                        </div>
                    </div>
...
function saveSettings(callback) {
    ...
    var url = 'index.php?route=extension/module/total_import/saveSettings&user_token=<?php echo $token ?>';
    $.ajax({
        type: "POST",
        url: url,
        data: data,
...
function saveAndExportSettings() {
    saveSettings(redirectForExporting);
}

function redirectForExporting() {
    window.location.href = 'index.php?route=extension/module/total_import/exportSettings&user_token=<?php echo $token ?>';
}
...
function showAlert(msg, alert_type) {
    ...
    $('#showAlert').prepend('<div class="alert alert-'+alert_type+'"><i class="fa fa-check-circle"></i> '+msg+'<button type="button" class="close" data-dismiss="alert">×</button></div>');
}
...
function next(cur_start, import_end, firstRun) {
    ...
    var var_limited_import = $('[name="limit_on_import"]').val();

    //Checking to see if its Numeric
    if ($.isNumeric(var_limited_import) ) {
        //Checking if the numeric value is sensible
        var_step_amount = Math.max(1,parseInt(var_limited_import));
   }
    ...
    $.ajax({
        url: 'index.php?route=extension/module/total_import/step5_ajax&user_token=<?php echo $token; ?>',
        type: 'post',
        dataType: 'json',
        ...
        success: function(json) {
            ...
            if (cur_end < import_end) {
                ...
                $.ajax({
                    url: 'index.php?route=extension/module/total_import/importEnd&user_token=<?php echo $token; ?>',
                    type: 'post',
                    dataType: 'json',
                    data: ajax_data,
                    success: function(json) {
...
$('#import_form').submit(function(event) {
    ...
    if (<?php if ($hj_dev) { echo "false";} else {echo "true";} ?>) {
        ...
        if (isFullImport) {
            var_start = 1;
            var_end = <?php echo $getNumItemsInFeed; ?>;
