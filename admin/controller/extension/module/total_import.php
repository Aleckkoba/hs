<?php
#####################################################################################
#  Module TOTAL IMPORT PRO for Opencart 3.x From HostJars opencart.hostjars.com     #
#####################################################################################
...
class ControllerExtensionModuleTotalImport extends Controller {
	private $version_number = 2.2;
	private $error = array();
	...
	*/
	public function index() {
		...
		$this->load->language('extension/module/total_import');
		...
		foreach ($pages as $page=>$title) {
			$data['pages'][$page] = array(
				'link'   => $this->url->link('extension/module/total_import/' . $page, 'user_token=' . $this->token, 'SSL'),
				'title'  => $title,
        ...
        $data['version_number'] = number_format($this->version_number,1);
		$data['ajax_action'] = $this->url->link('extension/module/total_import/delete_profile', 'user_token=' . $this->token, 'SSL');
		$data['settings_form_link'] = $this->url->link('extension/module/total_import/importSettings', 'user_token=' . $this->token, 'SSL');
		$this->load->model('extension/module/total_import');

		if ($this->model_extension_module_total_import->checkUpdates() > 0) {
        ...
        $data['db_exists'] = $this->model_extension_module_total_import->doesDatabaseExist();

        //load settings profile
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && !isset($this->session->data['was_settings_upload'])) {
			$profile_name = $this->request->post['settings_groupname'];
			if (!empty($profile_name)) {
				$this->model_extension_module_total_import->loadSettings($profile_name);
				...
				$this->response->redirect($this->url->link('extension/module/total_import', 'user_token=' . $this->token, 'SSL'));
			}
		}
		unset($this->session->data['was_settings_upload']);

		$data['php_path'] = PHP_BINDIR . '/php';
		$data['cron_index_path'] = DIR_APPLICATION . 'cron_index.php';
	...
	public function step1() {
		...
		if (defined('CLI_INITIATED')) {
			...
			$this->rotateLogs();
			$this->logger->write(sprintf('[%s] - [Cron] Profile Loaded: %s', $this->language->get('log_level_info'), PROFILE_NAME));
			$settings = $this->model_setting_setting->getSetting('import_step1');
			$settings = $this->unserializeSettings($settings);

			$filename = $this->model_extension_module_total_import->fetchFeed(
				$settings,
				isset($settings['unzip_feed']),
				(isset($settings['basic_auth']) && !empty($settings['user_basicauth'])) ?
				array(
					'user' => $settings['user_basicauth'],
					'pass' => $settings['pass_basicauth']
				) : false);
			if ($filename) {
				$this->model_extension_module_total_import->importFile($filename, $settings);
			}
			$this->step3();
			return;
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validate(1)) {
			...
			$filename = $this->model_extension_module_total_import->fetchFeed($this->request->post,
						isset($settings['unzip_feed']),
			...
			if ($this->validateFeed($filename, $settings)) {
				$import_status = $this->model_extension_module_total_import->importFile($filename, $settings);
				if ($import_status['total_items_ready'] == 0) {
					if($settings['format'] == 'csv') {
						$this->session->data['warning'] = $this->language->get('error_mac_csv');
					} else {
					...
					$this->response->redirect($this->url->link('extension/module/total_import/step2', 'user_token=' . $this->token, 'SSL'));
				}
			}

		}

		$_settings = $this->model_setting_setting->getSetting('import_step1');
		//Perform functions for every single page
		$data = $this->common(1);
		$data['can_skip'] = !empty($_settings);
	...
	public function step2() {
		...
		$this->language_info = array(
			...
			'entry_seo_keyword_category',   'entry_seo_keyword_category',
			'entry_seo_keyword_category_name','entry_seo_keyword_category_name',
			'entry_seo_keyword_blank',		'entry_seo_keyword_blank',
			'entry_related_field',			'entry_split_related',
			'text_field_model',				'text_field_sku',
			'text_field_upc',				'text_field_id',
			'entry_yes',					'entry_no',
			'entry_none_wide',				'help_image_subfolder',
			'help_remote_image',
		);
		...
		$data['image_folder_path'] = DIR_IMAGE . 'catalog/';
		$data['help_link'] = $this->help_2;
		$data['can_skip'] = $step_2_saved;
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validate(2)) {
			...
			$this->response->redirect($this->url->link('extension/module/total_import/step3', 'user_token=' . $this->token, 'SSL'));
		}
	...
	public function step3() {
		...
		if (defined('CLI_INITIATED')) {
			...
			if (isset($settings['adjust']) && is_array($settings['adjust'])) {
				$this->logger->write(sprintf('[%s] - [Cron] Running Adjustments (Step 3)', $this->language->get('log_level_info')));
				$this->model_extension_module_total_import->runAdjustments($settings['adjust']);
			}
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validate(3)) {
			...
			if (isset($this->request->post['adjust']) && is_array($this->request->post['adjust'])) {
				$this->model_extension_module_total_import->runAdjustments($this->request->post['adjust']);
			}

			$this->load->language('extension/module/total_import');
			$this->session->data['success'] = $this->language->get('text_success_step3');
			$this->response->redirect($this->url->link('extension/module/total_import/step4', 'user_token=' . $this->token, 'SSL'));
		...
		$data['feed_sample'] = $this->model_extension_module_total_import->getNextProduct();
		unset($data['feed_sample']['hj_id']);
		$data['fields'] = array_keys($data['feed_sample']);
		$data['help_link'] = $this->help_3;
		$data['operations'] = $this->model_extension_module_total_import->getOperations();
    ...
    public function getNextRow() {
       if (!$this->user->hasPermission('modify', 'extension/module/total_import')) {
            $this->error['warning'] = $this->language->get('error_permission');
        } else {
            $this->load->model('extension/module/total_import');
           $row = $this->model_extension_module_total_import->getNextProduct($this->request->post['nextRow']);
	...
	public function step4() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validate(4)) {
			...
			$multis = array('product_option', 'product_attribute', 'product_image', 'download', 'product_special', 'filter');
			...
			$this->response->redirect($this->url->link('extension/module/total_import/step5', 'user_token=' . $this->token, 'SSL'));
		}
		...
		if (!isset($settings['field_names'])) {
			...
			$data['field_names'] = array(
                ...
                'filter'=>array('hostjars')
			);
		} else {
			$data['field_names'] = $settings['field_names'];
			$multi_fields = array('product_attribute', 'product_option', 'product_image', 'product_discount', 'download', 'product_special', 'filter');
		...
		$data['field_map'] = array(
            ...
            //filters
            'filter' => array($this->language->get('text_field_filter'), 'vert'),
		...
		foreach ($all_languages as $lang) {
          if (isset($settings['language'])) {
            if (in_array($lang['language_id'], $settings['language'])) {
                $lang['image'] = 'language/' . $lang['code'] . '/' . $lang['code'] . '.png';
                $data['languages'][] = $lang;
            }
          }
		}
	...
	public function getNumItemsInFeed() {
		$this->load->model('extension/module/total_import');
		$this->response->setOutput($this->model_extension_module_total_import->getNumItemsInFeed());
	}

	public function download_log() {

		$trimmed_str = trim($_GET['selected_id'],'log');

		$attachment_location = DIR_LOGS . 'log.'. $trimmed_str;
  	...
  	public function log() {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('common/dashboard', 'user_token=' . $this->token, 'SSL')
        );
		$data['breadcrumbs'][] = array(
	   		'href'	  => $this->url->link('extension/module/total_import', 'user_token=' . $this->token, 'SSL'),
	   		'text'	  => 'Total Import PRO',
		);
		$data['breadcrumbs'][] = array(
	   		'href'	  => $this->url->link('extension/module/total_import/log', 'user_token=' . $this->token, 'SSL'),
	   		'text'	  => "Import Log",
		);

        $data['download_log'] = $this->url->link('extension/module/total_import/download_log', 'user_token=' . $this->token, 'SSL');
        ...
        $data['log'][] = array(
        		...
        		'tab_name' => date('d/M/Y',filemtime($file))
        	);
        ...
        $this->response->setOutput($this->load->view('extension/module/total_import_log', $data));
    }//End of Log()
		...
		if (!$this->user->hasPermission('modify', 'extension/module/total_import')) {
			...
			if ($this->validate(5)) {
				...
				$this->model_setting_setting->editSetting('import_step5', $settings);
				 ...
				 if (count($this->model_setting_setting->getSetting('import_step4')) === 0 ) {
				 	die('Please save your field mapping for your feed in step 4');
				 };
				...
				$num_items = $this->model_extension_module_total_import->getNumItemsInFeed();
	 ...
	 *
	 * @return json The Handler is in total_import_step5.twig
	 */
	public function importEnd() {
		if (!$this->user->hasPermission('modify', 'extension/module/total_import')) {
			...
			$this->load->language('extension/module/total_import');
			$this->load->model('extension/module/total_import'); // Needed to make $this work as expected, weird 2.0 change
	...
	private function deleteExistingProds($delete_diff, $update_field) {
		...
		$existing_prods = $this->model_extension_module_total_import->getExistingProds();
		...
		foreach ($existing_prods as $item_to_delete) {
			...
			elseif ($delete_diff == 'disable') {
				$this->model_extension_module_total_import->disableProduct($item_to_delete['product_id']);
			} else {
				$this->model_extension_module_total_import->zeroQuantityProduct($item_to_delete['product_id']);
			}
	...
	private function setupLogFiles() {
		$log_files = array(
			'log.1',
			'log.2',
			'log.3',
	...
	public function importSettings() {
		...
		if(!isset($_FILES['settings_file'])) {
			$this->session->data['warning'] = 'Error: You are trying to import the wrong file';
		}else {
		...
		if (isset($fileExtension[1]) && $fileExtension[1] !== 'csv')
		 	$this->session->data['warning'] = 'Error Importing Settings: Incorrect CSV format (try exporting again)';
		...
		if (empty($this->session->data['warning'])){
			...
			if(($handle =  fopen($file['tmp_name'],"r")) !== FALSE) {
				while (($line = fgets($handle)) !== false) {
					...
					//checking if there are two lines for each Setting, if there isn't the setting is just a blank line and won't import.
					if (count($split) === 2) {
						$key   = $split[0];
						$value = $split[1];
						preg_match('/^(import_step[1-5])_(([a-z]+)(_[a-z]+){0,})/', $key, $matches);
						if (!empty($matches))
							$settings[$matches[1]][$matches[0]] = $value;
					}
				}
			    	foreach ($settings as $code => $setting) {
						$this->model_setting_setting->editSetting($code, $setting);
				}
				$this->session->data['success'] = 'TIP Settings Imported Successfully!';
			    fclose($handle);
			}
			$this->session->data['was_settings_upload'] = true;
			$this->index();
		}
	}
    ...
    header("Content-Transfer-Encoding:  binary");
    header("Content-Length:  ".strlen($csv_headers.$csv_body));
	...
	public function step5() {
		...
		if (defined('CLI_INITIATED')) {
			...
			$this->logger->write(sprintf('[%s] - [Cron] Importing running..', $this->language->get('log_level_info')));
			$this->import($settings);
			$this->logger->write(sprintf('[%s] - [Cron] Import done. Products Added: %s, Products Updated: %s', $this->language->get('log_level_info'), $this->total_items_added, $this->total_items_updated));
		...
		$this->language_info = array(
			...
			'entry_limit_on_import', 'help_products_per_segment',
		);
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validate(5)) {
			...
			if (!empty($this->request->post['save_settings_name'])) {
				$this->load->model('extension/module/total_import');
				$this->model_extension_module_total_import->saveSettings($this->request->post['save_settings_name']);
			}
			$settings = array_merge(
				$this->model_setting_setting->getSetting('import_step1'),
				$this->model_setting_setting->getSetting('import_step2'),
			...
			$this->logger->write(sprintf("[%s] - Total Products added: %s", $this->language->get('log_level_info'), $this->total_items_added));
			$this->logger->write(sprintf("[%s] - Total Products updated: %s", $this->language->get('log_level_info'), $this->total_items_updated));
			$this->response->redirect($this->url->link('catalog/product', 'user_token=' . $this->token, 'SSL'));
		...
		if (!empty($settings['limit_on_import']))
			$data['limit_on_import'] = $settings['limit_on_import'];

		$data['getNumItemsInFeed'] = $this->model_extension_module_total_import->getNumItemsInFeed();

		$data['backup_link'] = $this->url->link('tool/backup/', 'user_token=' . $this->token, 'SSL');
		$data['export_settings'] = $this->url->link('extension/module/total_import/exportSettings', 'user_token=' . $this->token, 'SSL');
		$this->response->setOutput($this->load->view($this->template, $data));
		// mark the stop time
	...
	public function delete_profile() {
		...
		if ($this->validate()) {
			...
			if ($profile_name) {
				$this->load->model('extension/module/total_import');
				$this->model_extension_module_total_import->deleteSettings($profile_name);
	...
	*/
	private function common($step='') {
		...
		//gather user info for intercom
		$this->load->model('user/user');
		$user = $this->model_user_user->getUser($this->session->data['user_id']);
		$data['user_name'] = $user['firstname'].' '.$user['lastname'];
		$data['user_email'] = $user['email'];
		$data['saved_settings'] = $this->model_extension_module_total_import->getSavedSettingNames();
		$data['preset_settings'] = $this->model_extension_module_total_import->getSavedPresetSettingNames();
		...
		$page = ($step) ? 'extension/module/total_import/step' . $step : 'extension/module/total_import';

		//header
		$this->config->set('template_engine', 'template');

		$data['header_view'] = $this->load->view('extension/module/total_import_header', $data);

		// Form Submit Action
		$data['skip_url'] = $this->url->link('extension/module/total_import/step' . ($step+1), 'user_token=' . $this->token, 'SSL');
		$data['back_url'] = $this->url->link('extension/module/total_import/step' . ($step-1), 'user_token=' . $this->token, 'SSL');
		$data['action'] = $this->url->link($page, 'user_token=' . $this->token, 'SSL');
		$data['cancel'] = $this->url->link('extension/module/total_import', 'user_token=' . $this->token, 'SSL');
		
		$this->template = ($step) ? 'extension/module/total_import_step' . $step : 'extension/module/total_import';
	...
	*/
	private function import($settings) {
		...
		$this->model_extension_module_total_import->emptyTables($table_list);
		...
		if (!$table_list['product'] && $delete_diff != 'ignore' && isset($settings['first_run'])) {
				$this->model_extension_module_total_import->createDeleteDiffTable($settings['update_field'], $settings);
		}

		//Prepare Options if necessary
		$options = $this->model_extension_module_total_import->getOptions($settings['field_names']['product_option']);
		...
		while (($raw_prod = $this->model_extension_module_total_import->getNextProduct($product_num)))
		{
			$product_num++;
			...
			if (isset($settings['field_names']['minimum'])) {
				if (isset($raw_prod[$settings['field_names']['minimum']])) {
					if (strlen(trim($raw_prod[$settings['field_names']['minimum']])) === 0)
						unset($raw_prod[$settings['field_names']['minimum']]);
				}
			}
			...
			foreach (array_merge($settings['field_names']['product_image'], array($settings['field_names']['image'])) as $image) {
				if (!empty($raw_prod[$image])) {
					...
					if ($settings['remote_images']) {
						...
						$image_fetch = $this->model_extension_module_total_import->fetchImage($raw_prod[$image], $settings['image_subfolder']);

                                                if (!empty($image_fetch['info'])) {
                                                        $this->logger->write(sprintf("[%s] - Image downloaded: %s, Size: %s, Time Taken: %s secs", $this->language->get('log_level_info'), $image_fetch['filename'], $this->model_extension_module_total_import->human_filesize($image_fetch['info']['size_download']), $image_fetch['info']['total_time']));
                                                }
                                                if (!empty($image_fetch['error'])) {
                                                        $this->logger->write($image_fetch['error']);
                                                }
                                                $filename = $image_fetch['filename'];
                                        } else {
						$image_data = $this->model_extension_module_total_import->validateImageFile($raw_prod[$image]);

                                                if (!empty($image_data['error'])) {
                                                        $this->logger->write($image_data['error']);
                                                }
                                                $filename = $image_data['filename'];
					}

					$raw_prod[$image] = $filename;
				}
			}
			...
			if (isset($settings['update_field'])) {
				...
				if (isset($update_value))
					$update_id = $this->model_extension_module_total_import->getProductId($settings['update_field'], $update_value);
			}
			///EXISTING PRODUCT:
			if ($update_id && $settings['existing_items'] != 'skip') {
				...
				if($settings['simple']) {
					...
					if (count($simple_update) >= 1) {
						$this->model_extension_module_total_import->simpleUpdate($update_id, $simple_update);
			...
			if (!$table_list['product'] && $delete_diff != 'ignore') {
				if (isset($raw_prod[$settings['field_names'][$settings['update_field']]]))
					$this->model_extension_module_total_import->deleteExistingProdHash($raw_prod[$settings['field_names'][$settings['update_field']]]);
			}
	...
	private function addProduct($raw_prod, $settings) {
        ...
        if (!empty($settings['field_names']['filter'][0])) {
            $input_filters = array();
		...
		if (!empty($raw_prod[$settings['field_names']['product_related']])) {
			...
			foreach ($related_products as $related_product){
				if ($related_id = $this->model_extension_module_total_import->getProductId($settings['related_field'], trim($related_product))){
		...
		if (!empty($downloads)) {
			...
			$product['product_download'] = array_unique($downloads);
		}
		...
		if(!(in_array($this->config->get('config_language_id'),$settings['language']))) {
			$settings['language'][] = $this->config->get('config_language_id');
		}
		...
		foreach ($this->desc_data as $field => $default_value) {
			foreach ($settings['language'] as $language) {
				if (isset($settings['field_names'][$field][$language]) && isset($raw_prod[$settings['field_names'][$field][$language]])) {
					...
					if ($field == 'name')
						$product['product_description'][$language][$field] = $this->encodeProductName($raw_prod[$settings['field_names'][$field][$language]]);
				} else {
	...
	private function updateProduct($update_id, $raw_prod, $settings) {
		...
		if (!empty($settings['field_names']['product_image'][0])) {
			$product['product_image'] = array();
			foreach ($settings['field_names']['product_image'] as $image) {
				if (!empty($raw_prod[$image])) {
                    $product['product_image'][] = array('sort_order' => '', 'image' => $raw_prod[$image]);
	...
	//downloads
		...
		foreach ($this->desc_data as $field => $default_value) {
			foreach ($settings['language'] as $language) {
				if (isset($settings['field_names'][$field][$language]) && isset($raw_prod[$settings['field_names'][$field][$language]])) {
					...
					if ($field == 'name')
						$product['product_description'][$language][$field] = $this->encodeProductName($raw_prod[$settings['field_names'][$field][$language]]);
				} elseif (!isset($product['product_description'][$language][$field])) {
					$product['product_description'][$language][$field] = $default_value;
				}
			}
		}
		...
		//Product Specials
		if (!empty($settings['field_names']['product_special'][0])){ //product special fields mapped
			$product_special = $this->getSpecials($raw_prod, $settings);
			if (!empty($product_special)) {
				...
				$this->model_extension_module_total_import->deleteSpecials($update_id);
			}
		}
		else{
	...
	private function encodeProductName($name_string) {
	...
	private function getDiscounts($raw_prod, $settings) {
		...
		foreach ($settings['field_names']['product_discount'] as $discount) {
			if (isset($raw_prod[$discount]) && !empty($raw_prod[$discount])) {
				...
				if (empty($discount_parts[2])) {
					$discount_parts[2] = 'default';
				}

				if ($discount_parts[2] == '*') {
					foreach ($settings['customer_group_ids'] as $group_id) {
						$discounts[] = array(
			...
			foreach ($downloads as $dow) {
				if ($dow != '') {
					...
					if(isset($dow_parts[1])) {
						$dow_id = (int)$this->model_extension_module_total_import->getDownloadIdByName($dow_parts[1]);
					}
					else
					{
						$dow_id = (int)$this->model_extension_module_total_import->getDownloadIdByName($dow_parts[0]);
					}
	...
	private function getCategories($raw_prod, $settings) {
		...
		foreach ($settings['field_names']['category'] as $category_field) {
			...
			foreach ($categories as $cat) {
				$cat = trim($cat);
				if ($cat != '') {
					$cat_id = (int)$this->model_extension_module_total_import->getCategoryId($cat, $parentid);
					if ($cat_id == 0) {
						//doesn't exist so add it then get it's id
						$new_cat = array();
						...
						if(isset($settings['seo_keyword']) && $settings['seo_keyword'] === 'categoryName') {
							$new_cat['keyword'] = preg_replace('/[^a-zA-Z0-9]+/', '-',$cat);
						}
						...
						$cat_id = $this->model_catalog_category->addCategory($new_cat);
					}
					$temp_cat[] = $cat_id;
					$parentid = $cat_id;
				}
			...
			$new_cat = $temp_cat;
			if ($settings['bottom_category_only']) {
				$new_cat = array();
				if ($parentid != 0)
					$new_cat = array($parentid);
			}
	...
	private function getFilters($input_filters, $settings) {
		...
		foreach ($input_filters as $name=>$value) {
			...
			$filt_group_id = $this->model_extension_module_total_import->getFilterGroupId($name);
			if ($filt_group_id == 0) {
				...
				if (!empty($value)){
					...
					$filt_group_id = $this->model_extension_module_total_import->getFilterGroupId($name);
					$filt_name_id = $this->model_extension_module_total_import->getFilterNameId($value, $filt_group_id);
				}
			} else{
	...
	private function getManufacturer($manu, $settings) {
		$manu_id = $this->model_extension_module_total_import->getManufacturerId($manu);
		if ($manu_id == 0) {
			...
			$manu_id = $this->model_extension_module_total_import->getManufacturerId($new_manu['name']);
		}
		return $manu_id;
	}

	private function getStockStatus($stock, $settings) {
		$stock_id = $this->model_extension_module_total_import->getStockStatusId($stock);
		if ($stock_id == 0) {
			...
			$stock_id = $this->model_extension_module_total_import->getStockStatusId($new_stock['stock_status'][$settings['language'][0]]['name']);
		}
		return $stock_id;
	}
	...
	private function getAttributes($input_attributes, $settings) {
		...
		foreach ($input_attributes as $attr_name=>$attr_value) {
			...
			$attr_group_id = $this->model_extension_module_total_import->getAttributeGroupId($group);
			if ($attr_group_id == 0) {
			...
			$attr_id = $this->model_extension_module_total_import->getAttributeId($name, $attr_group_id);
			if ($attr_id == 0) {
				//it doesn't exist, let's add it
	...
	*/
	private function getProductOptions($raw_prod, $settings) {
		...
		foreach ($settings['field_names']['product_option'] as $option_field) {
			if (!empty($raw_prod[$option_field])) {
				// Trim whitespace from all values, then remove duplicates
				$options = array_unique(array_map('trim', explode('|', $raw_prod[$option_field])));
				if (!empty($options)) {
					$option_id = $this->model_extension_module_total_import->getOptionIdByName(ucwords($option_field));
					...
					foreach ($options as $option) {
						...
						if($option_parts[0] != '') {
							...
							$option_value_id = $this->model_extension_module_total_import->getOptionValueIdByName($option_parts[0], $option_id);
							$complete_option[$i]['product_option_value'][] = array(
		...
		$filtered_empty_values = array();
		foreach ($complete_option as $option) {
			if (!empty($option['product_option_value']))
				$filtered_empty_values[] = $option;
		}
		return $filtered_empty_values;
	}
	...
	private function createOption($option_name, $option_values, $settings) {
		...
		if (!empty($option_values)) {
			...
			$option_id = $this->model_extension_module_total_import->getOptionIdByName(ucwords($option_name));
			$new_option = array();
			foreach ($settings['language'] as $language) {
	...
	private function validate($step='') {
		...
		} elseif (!$this->user->hasPermission('modify', 'extension/module/total_import')) {
			$this->error['warning'] = $this->language->get('error_permission');
		} elseif ($step) {
			$this->load->model('extension/module/total_import');
			if ($step == '1') {
				...
				if (!empty($settings['source']) && ($settings['source'] == 'filepath')) {
					if (!file_exists($settings['feed_filepath'])) {
						$this->error['warning'] = $this->language->get('error_file_source');
					}
				}
	...
	private function validateFeed($filename, $settings) {
		...
		if (!$filename) {
			if(!isset($settings['source'])) {
				$this->error['warning'] = $this->language->get('Uploaded File is to large, please increase the upload_max_filesize and post_max_size in your PHP config');
			} else {
				if ($settings['source'] == 'file' && $this->request->files['feed_file']['error'] !== UPLOAD_ERR_OK) {
					$this->error['warning'] = $this->model_extension_module_total_import->fileUploadErrorMessage($this->request->files['feed_file']['error']);
				} else {
	...
	public function saveSettings() {
		if(isset($this->request->post['step'])) {
			...
			$this->load->language('extension/module/total_import');
			if (!empty($this->request->post['save_settings_name']))
				print $this->language->get('text_success').' as '.$this->request->post['save_settings_name'];
			else
