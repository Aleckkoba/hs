...
class ModelExtensionModuleTotalImport extends Model
{
  ...
  public function doesDatabaseExist() {
    ...
    $db_setting = $this->model_setting_setting->getSetting('tip_db_created');
    if (!empty($db_setting['tip_db_created_db_ready']))
    	return (unserialize($db_setting['tip_db_created_db_ready']) == 1);
    else
    	return false;
  }
			...
			if ($option) {
				...
				foreach ($query->rows as $row) {
					...
					foreach ($opt_values as $opt_value) {
						...
						if (!empty($opt_value_details[0]) && !in_array($opt_value_details[0], $exists)) {
	...
	public function getOperations($hide_func=true) {
		$operations = array(
			'addFieldToField' => array(
				'name' => $this->language->get('operation_add_field_to_field'),
				'function' => ($hide_func) ? '' : 'addFieldToField',
				'inputs'=>array(
					array('type'=>'field', 'prepend'=>$this->language->get('operation_add')),
					array('type'=>'field', 'prepend'=>$this->language->get('operation_to')),
				),
				'label' => $this->language->get('operation_label_most_popular'),
			),
			'multiplyFieldByField' => array(
				'name' => $this->language->get('operation_multiply_field_by_field'),
				'function' => ($hide_func) ? '' : 'multiplyFieldByField',
				'inputs'=>array(
					array('type'=>'field', 'prepend'=>$this->language->get('operation_multiply')),
					array('type'=>'field', 'prepend'=>$this->language->get('operation_by')),
				),
				'label' => $this->language->get('operation_label_most_popular'),
			),
			'adjustPercentage' => array(
				'name' => $this->language->get('operation_adjust_percentage'),
				'function' => ($hide_func) ? '' : 'adjustPercentage',
				'helptext' => 'e.g 10% to add 10%, -10% to subtract 10%.',
				'inputs'=>array(
					array('type'=>'field', 'prepend'=>$this->language->get('operation_adjust')),
			...
			'mergeOptionColumns' => array(
				'name' => $this->language->get('operation_merge_option_columns'),
			   	'function' => ($hide_func) ? '' : 'mergeOptionColumns',
			   	'helptext' => 'This will make a new column for your Options. Merged fields will be appended to the base column separarated by ":", and each option separated by "|".',
				'inputs'=>array(
					array('type'=>'text', 'prepend'=>$this->language->get('operation_option_name')),
					array('type'=>'field', 'prepend'=>$this->language->get('operation_base_column')),
	...
	public function appendText($adjustment) {
		...
		$field = $adjustment[1];
		$this->db->query('UPDATE ' . DB_PREFIX . "hj_import SET `" .$field . "` = CONCAT( `" . $field . "`, '" . $this->db->escape($append_text) . "' ) WHERE 1");
	}
	...
	public function prependText($adjustment) {
		...
		$field = $adjustment[1];
		$this->db->query('UPDATE ' . DB_PREFIX . "hj_import SET `" . $field . "` = CONCAT( '" . $this->db->escape($prepend_text) . "', `" . $field . "` ) WHERE 1");
	}
	...
	*/
	...
	/**
	* @param (mixed) array(field to multiply, field to multiply by)
	*/
	public function multiplyFieldByField($adjustment) {
		$field_to_multiply = $adjustment[0];
		$multiplier = $adjustment[1];
		$this->db->query('UPDATE ' . DB_PREFIX . "hj_import SET `" . $field_to_multiply . "` = (cast(`" . $field_to_multiply . "`  as DECIMAL(10,2)) * cast(`" . $multiplier . "` as DECIMAL(10,2)) )");
	}

	/**
	* @param (mixed) array(field to multiply, field to multiply by)
	*/
	public function addFieldToField($adjustment) {
		$field_to_add = $adjustment[0];
		$field = $adjustment[1];
		$this->db->query('UPDATE ' . DB_PREFIX . "hj_import SET `" . $field . "` = (cast(`" . $field . "` as DECIMAL(10,2)) + cast(`" .  $field_to_add . "`  as DECIMAL(10,2)) )");
	}

	public function adjustPercentage($adjustment) {
		$field = $adjustment[0];
		$percentage = str_replace('%', '', $adjustment[1]);
		if (is_numeric($percentage)) {
			$percentage = (100.00 + (float) $percentage) / 100.00;
			$this->db->query('UPDATE ' . DB_PREFIX . "hj_import SET `" . $field . "` = (cast(`" . $field . "` as DECIMAL(10,2)) * " . $percentage . " )");
		}
	}
...
//	}
	...
	* @param (mixed) array(field to adjust, text to look for)
	*/
	public function mergeOptionColumns($adjustment) {
		$option_name = $adjustment[0];
		$base_column = $adjustment[1];
		$adjustment_length = count($adjustment);
		$index = 2;
		while ($index < $adjustment_length) {
			$columns_to_merge[] = $adjustment[$index];
			$index++;
		}

		if (!$this->columnExists(DB_PREFIX.'hj_import', $option_name)) {
			$this->alterImportTable(array($option_name));
		}

		$sql = 'SELECT `hj_id`, `' . $base_column . '`, `' . implode('`, `', $columns_to_merge) . '` FROM `' . DB_PREFIX . 'hj_import`';
		$query = $this->db->query($sql);

		foreach($query->rows as $row) {
			$merged_value = '';

			$base_split = explode('^', $row[$base_column]);
			$num_options = count($base_split);

			$merge_columns_split = array();
			foreach ($columns_to_merge as $column_name) {
				$merge_columns_split[] = explode('^', $row[$column_name]);
			}

			$index = 0;
			while ($index < $num_options) {
				$merged_value .= $base_split[$index];
				foreach ($merge_columns_split as $split_column) {
					$merged_value .= ':' . $split_column[$index];
				}
				$index++;
			...
			$sql = 'UPDATE `' . DB_PREFIX . "hj_import` SET `" . $this->db->escape($option_name) . "` = '". $this->db->escape($merged_value) . "' WHERE `hj_id` = '" . $this->db->escape($row['hj_id']) . "'";
			$query = $this->db->query($sql);
		}
	}
	...
	*/
	...
	public function createEmptyTable($headings) {
		...
		$this->load->model('setting/setting');
		$this->model_setting_setting->editSetting('tip_db_created', array('tip_db_created_db_ready' => serialize(1)));
	}
	...
	public function createPresetTable() {
		...
		$sexshop365_settings = array(
			...
			'import_step2_out_of_stock_status' => 's:1:"6";',
			'import_step2_step' => 's:12:"import_step2";',
			'import_step3_adjust' => 'a:15:{i:1;a:3:{i:0;s:7:"prepend";i:1;s:5:"catalog/";i:2;s:17:"Product Thumbnail";}i:2;a:3:{i:0;s:7:"prepend";i:1;s:5:"catalog/";i:2;s:17:"Product Big Image";}i:3;a:3:{i:0;s:7:"prepend";i:1;s:5:"catalog/";i:2;s:19:"Small Multi Image 1";}i:4;a:3:{i:0;s:7:"prepend";i:1;s:5:"catalog/";i:2;s:19:"Small Multi Image 2";}i:5;a:3:{i:0;s:7:"prepend";i:1;s:5:"catalog/";i:2;s:19:"Small Multi Image 3";}i:6;a:3:{i:0;s:7:"prepend";i:1;s:5:"catalog/";i:2;s:19:"Small Multi Image 4";}i:7;a:3:{i:0;s:7:"prepend";i:1;s:5:"catalog/";i:2;s:17:"Big Multi Image 1";}i:8;a:3:{i:0;s:7:"prepend";i:1;s:5:"catalog/";i:2;s:17:"Big Multi Image 2";}i:9;a:3:{i:0;s:7:"prepend";i:1;s:5:"catalog/";i:2;s:17:"Big Multi Image 3";}i:10;a:3:{i:0;s:7:"prepend";i:1;s:5:"catalog/";i:2;s:8:"XL Image";}i:11;a:3:{i:0;s:7:"prepend";i:1;s:5:"catalog/";i:2;s:10:"XL Image 2";}i:12;a:3:{i:0;s:7:"prepend";i:1;s:5:"catalog/";i:2;s:10:"XL Image 3";}i:13;a:3:{i:0;s:7:"prepend";i:1;s:5:"catalog/";i:2;s:10:"XL Image 4";}i:14;a:3:{i:0;s:7:"prepend";i:1;s:5:"catalog/";i:2;s:10:"XL Image 5";}i:15;a:3:{i:0;s:7:"prepend";i:1;s:5:"catalog/";i:2;s:5:"Image";}}',
			'import_step3_step' => 's:12:"import_step3";',
	...
	public function deleteSettings($group) {
		if (strpos($group, 'preset_') === 0) {
			$this->db->query("DELETE FROM " . DB_PREFIX . "hj_preset_settings WHERE `group` = '" . $this->db->escape(str_replace('preset_', '', $group)) . "'");
		} else {
			$this->db->query("DELETE FROM " . DB_PREFIX . "hj_import_settings WHERE `group` = '" . $this->db->escape($group) . "'");
		}
	}
	...
	public function fetchFeed($settings, $unzip_feed=false, $use_auth=false) {
		$this->logger->write(sprintf('[%s] - Starting feed fetch.', $this->language->get('log_level_info')));
		$success = false;
		$filename = DIR_UPLOAD . 'feed.txt';
		if(isset($settings['source'])) {
			$this->logger->write(sprintf('[%s] - Feed Source: %s', $this->language->get('log_level_info'), $settings['source']));
			if ($settings['source'] == 'file') {
				...
				$this->logger->write(sprintf('[%s] - %s', $this->language->get('log_level_info'), 'Feed URL: '.$settings['feed_url']));
				$ch = curl_init();
				$fp = fopen($filename, 'w');
				...
				$this->logger->write(sprintf("[%s] - Feed downloaded. Size: %s, Time Taken: %s secs", $this->language->get('log_level_info'), $this->human_filesize($curl_info['size_download']), $curl_info['total_time']));
				curl_close($ch);
				fclose($fp);
	 ...
	 *
	 * @desc - Validates image filename by checking that the file exists and
	 * trying common folders to check if image is there. See #131
	 *
	 * @param $filename(string) The image filename.
	 * @return (string) the name of the image file on disk relative to the image/ dir or empty string.
	 *
	 */
	public function validateImageFile($filename) {
		$result = array(
			'filename' => $filename,
			'error' => ''
		);
		...
		if (file_exists($filename)) {
			$result['filename'] = str_replace(DIR_IMAGE, '', $filename);
			$result['error'] = 'Image path auto corrected. : ' . $result['filename'];
			return $result;
		}

		if (strpos($filename, 'image/') === 0) {
			$result['filename'] = str_replace('image/', '', $filename);
			$result['error'] = 'Image path auto corrected. : ' . $result['filename'];
		} elseif (strpos($filename, 'data/') === 0) {
			$result['filename'] = str_replace('data/', 'catalog/', $filename);
			$result['error'] = 'Image path auto corrected. : ' . $result['filename'];
		} elseif (strpos($filename, 'catalog/') === 0) {
			$result['filename'] = str_replace('catalog/', 'data/', $filename);
			$result['error'] = 'Image path auto corrected. : ' . $result['filename'];
		} else {
			$result['error'] = 'Image file does not exist. : ' . $filename;
			$result['filename'] = '';
		}

		if (!file_exists(DIR_IMAGE . $result['filename'])) {
			$result['error'] = 'Image file does not exist. : ' . $result['filename'];
			$result['filename'] = '';
		}
	...
	public function fetchImage($image_url, $folder='') {
		...
		if (empty($error)) {
			...
			if (strstr($image_url, '?')) {
				...
				if($folder != '' && $folder != '/') {
					$filename = 'catalog/' . $folder . '/' . md5($image_url) . '.' . $image_type;
				} else {
					$filename = 'catalog/' . md5($image_url) . '.' . $image_type;
				}
			} else {
	...
	*/
	...
	public function checkUrlAlias($text) {
		...
		return (isset($query->row['keyword']));
	}
